...
# welcome. Only used at game-start, and data that is unchanged during the game
# create. Creates the different objects etc the player can interact with.
# Some will be altered during game.
# functions. The main game-logic is in this import.

import functions
# main game logic is in this file
import welcome
# functions for initial greetings, help-texts
import create
...
while game_in_progress:
    ...
    # parser function splits the input string into words and returns it here
    if user_string is not None:
        # if none, loop back to new prompt
        verb = user_string[0]
        noun = user_string[1]
        value = functions.check_input(verb, noun, name)
        # check for valid inputs, and processing proceeds in functions.py file.
        # Name is passed to be able to customize some string with username
        if value == 50:
            # if the input is quit, return 50 to terminate the game
            print("---------------------")
